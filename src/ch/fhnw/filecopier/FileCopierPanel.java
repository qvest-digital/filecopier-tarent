...
public class FileCopierPanel extends JPanel implements PropertyChangeListener {
    ...
    private long previousCopied;
    private long previousMissing;
...
//                FileCopier.BYTE_COUNTER_PROPERTY, this);
    ...
    private void update() {
        ...
        setVisibleChangeDataVolumeLabel(
                transferredDataVolumeLabel, previousCopied, bytesCopied);
        previousCopied = bytesCopied;
        ...
        long missing = byteCount - bytesCopied;
        setVisibleChangeDataVolumeLabel(
                missingDataVolumeLabel, previousMissing, missing);
    ...
    private void setVisibleChangeDataVolumeLabel(
            JLabel label, long previousVolume, long newVolume) {
        
        int maxFractionDigits = 0;
        // compare the order of magnitudes
        double previousOrder = Math.floor(Math.log10(previousVolume));
        double newOrder = Math.floor(Math.log10(newVolume));
        if (previousOrder == newOrder) {
            // compare integer values
            int previousInt = getSubKiloInt(previousVolume);
            int newInt = getSubKiloInt(newVolume);
            if (previousInt == newInt) {
                // both values have the same order of magnitude and the same
                // integer value -> show fraction digits so that a change is
                // visible
                maxFractionDigits = 3;
            }
        }
        
        label.setText(getDataVolumeString(newVolume, maxFractionDigits));
    }
